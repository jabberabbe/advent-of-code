...
struct Bag<'a> {
    ...
    contains: RefCell<Vec<(usize, &'a Bag<'a>)>>,
}
    ...
    for bag_rule in buffer.split("\n\n").filter(|s| !s.is_empty()) {
        ...
        for color in iter {
            if color != "no other" {
                ...
                let contained_number = color[..1].parse::<usize>().unwrap();
                parent_bag.contains.borrow_mut().push((contained_number, contained_bag));
            }
        }
    }

    let bag = bag_colors.get("shiny gold").unwrap();
    println!("{}", count_multiple_descendants(bag));
}
...
fn get_or_create_bag<'a>(color: &'a str, map: &mut HashMap<&'a str, &'a Bag<'a>>,
    ...
    match map.get(color) {
        ...
        None => {
            let bag_ref = arena.alloc(Bag { contains: RefCell::new(Vec::new()) });
...
fn count_multiple_descendants<'a>(node: &Bag<'a>) -> usize {
    let mut count = 0;
    for (n, son) in node.contains.borrow().iter() {
        count += n * (count_multiple_descendants(son) + 1);
    }
    return count;
}
